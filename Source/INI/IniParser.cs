...
namespace ReiPatcher.INI
{
    ...
    {
        #region Fields
        private static Regex CommentRegex = new Regex(@"^;(?<com>.*)");
        private static Regex KeyRegex = new Regex(@"^(?<key>[\w\s]+)=(?<val>.*)$");
        private static Regex SectionRegex = new Regex(@"^\[(?<sec>[\w\s]+)\]$");
        private static Regex VarRegex = new Regex(@"^\@(?<key>[\w\s]+)=(?<val>.*)$");
        #endregion
