...
namespace ReiPatcher
{
    partial class Program
    {
        private static void RunPatchers(PatchBase[] patchers, params PatcherArguments[] assemblies)
        {
            ...
            {
                ConsoleUtil.Print($"Assembly '{def.Assembly.Name.Name}'", Console.BufferWidth, true, '-', ConsoleColor.DarkGreen);
                foreach (var patcher in patchers)
                {
                    ConsoleUtil.Print($"Patcher: '{patcher.Name}'", Console.BufferWidth, true, '-', ConsoleColor.DarkGray);
                    try
                    {
        ...
        private static void RunSaveAndBackup(PatcherArguments[] assemblies)
        {
            foreach (var ass in assemblies)
            {
                ...
                {
                    ConsoleUtil.Print($"Not Patched '{Path.GetFileName(ass.Location)}'", color: ConsoleColor.DarkYellow);
                    continue;
                }
                ...
                {
                    var destFileName = string.Format("{0}.{1:" + BACKUP_DATE_FORMAT + "}.bak", ass.Location, DateTime.Now);

                    ConsoleUtil.Print($"Creating Assembly Backup: '{Path.GetFileName(destFileName)}'",
                        color: ConsoleColor.DarkYellow);

                    File.Copy(ass.Location, destFileName);
                }
                if (attrs.None(attribute => attribute.Info == "ReiPatcher"))
